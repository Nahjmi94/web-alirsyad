...
export default function SemuaPrestasiSmpPage() {
  const daftarPrestasi = [
    { id: '1', juara: 'Medali Emas OSN IPA', event: 'Tingkat Provinsi Jawa Tengah', foto: 'https://placehold.co/600x400/041a12/fbbf24?text=Prestasi+SMP+1', desc: 'Perwakilan siswa kita sukses mengungguli ratusan peserta lain di ajang Olimpiade Sains Nasional bidang IPA.' },
    { id: '2', juara: 'Juara 1 Story Telling', event: 'English Competition 2026', foto: 'https://placehold.co/600x400/041a12/fbbf24?text=Prestasi+SMP+2', desc: 'Penghayatan dan pronounciation yang sempurna mengantarkan ananda meraih piala bergilir tingkat kota.' },
    { id: '3', juara: 'Juara Umum Pramuka Penggalang', event: 'Jambore Cabang Pekalongan', foto: 'https://placehold.co/600x400/041a12/fbbf24?text=Prestasi+SMP+3', desc: 'Tim regu inti Pramuka SMP Al-Irsyad berhasil memborong piala di berbagai kategori lomba ketangkasan.' },
    { id: '4', juara: 'Juara 2 MTQ Pelajar', event: 'Tingkat SMP se-Pekalongan', foto: 'https://placehold.co/600x400/041a12/fbbf24?text=Prestasi+SMP+4', desc: 'Lantunan ayat suci Al-Qur\'an yang indah berhasil menyentuh hati para juri pada ajang MTQ tahunan.' }
  ];
    ...
    <div className="flex flex-col min-h-screen bg-[#071E16] text-slate-100 pb-20 font-sans">
      <section className="relative pt-12 pb-10 px-6 text-center border-b border-emerald-900/50">
        <RevealOnScroll>
          ...
          <h1 className="text-3xl md:text-5xl font-black text-white mb-4 drop-shadow-md">Arsip Prestasi Siswa SMP</h1>
          <p className="text-emerald-100/70 max-w-2xl mx-auto text-sm sm:text-base font-light">Torehan tinta emas yang membuktikan kualitas pendidikan akademik dan non-akademik di SMP Al-Irsyad Pekalongan.</p>
        </RevealOnScroll>
      </section>
              ...
              <div className="bg-[#041a12] border border-emerald-800/50 rounded-3xl overflow-hidden hover:-translate-y-2 hover:border-amber-400/50 transition-all duration-500 flex flex-col h-full relative group">
                ...
                <div className="p-6 flex flex-col flex-1 justify-between space-y-4 bg-[#041a12]">
                  ...
                  <div className="pt-4 border-t border-emerald-800/40">
                    <Link href={`/smp/prestasi/${item.id}`} className="inline-flex items-center gap-2 text-sm font-bold text-amber-400 hover:text-amber-300 transition-colors group/link">
